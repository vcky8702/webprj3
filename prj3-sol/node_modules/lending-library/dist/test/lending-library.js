...
describe('lending library', () => {
    ...
    describe('findLendings() with populated library', async () => {
        beforeEach(async () => {
            for (const book of BOOKS) {
                const bookResult = await library.addBook(book);
                assert(bookResult.isOk === true);
        ...
        it('must find all checkouts of books by a isbn', async () => {
            ...
            const lendsResult = await library.findLendings({ findBy: 'isbn', isbn });
            assert(lendsResult.isOk === true);
            const lends = lendsResult.val.sort((b1, b2) => b1.patronId.localeCompare(b2.patronId));
        ...
        it('must find all checkouts of books by a patronId', async () => {
            ...
            const lendsResult = await library.findLendings({ findBy: 'patronId', patronId });
            assert(lendsResult.isOk === true);
            const lends = lendsResult.val.sort((b1, b2) => b1.isbn.localeCompare(b2.isbn));
